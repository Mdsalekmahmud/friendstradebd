...
(function () {
    function a() { h = !0; d.removeEventListener("DOMContentLoaded", a); f.removeEventListener("load", a); g.forEach(function (c) { c() }) } function b(c) { return parseFloat(c ? c : 0) } var d = document, f = window, g = [], h = !1; "complete" === d.readyState ? a() : (d.addEventListener("DOMContentLoaded", a), f.addEventListener("load", a)); var l = Array.prototype.slice; String.prototype.startsWith || (String.prototype.startsWith = function (c, a) { return this.substr(!a || 0 > a ? 0 : +a, c.length) === c }); String.prototype.endsWith || (String.prototype.endsWith =
        function (c, a) { if (void 0 === a || a > this.length) a = this.length; return this.substring(a - c.length, a) === c }); f.$ = function (c, a) {
            if (c instanceof Function) return h ? c() : g.push(c), d; if (c instanceof NodeList) return new JQLite(l.call(c)); if (c instanceof Node && 1 === c.nodeType) return new JQLite([c]); var b; if (b = "string" === typeof c) b = c.trim(), b = "<" === b[0] && ">" === b[b.length - 1] && 3 <= b.length; if (b) {
                c = c.trim(); if (b = c.match(/^<([\w-]+)\s*\/?>(?:<\/\1>|)$/)) {
                    a = a || {}; c = d.createElement(b[1]); for (var k in a) a.hasOwnProperty(k) &&
                        c.setAttribute(k, a[k]); c = [c]
                } else c.startsWith("<!DOCTYPE") || c.startsWith("<!doctype") ? c = [(new DOMParser).parseFromString(c, "text/html").documentElement] : (a = d.createElement("div"), a.innerHTML = c, c = l.call(a.childNodes)); return new JQLite(c)
            } if ("string" === typeof c) return new JQLite(l.call(d.querySelectorAll(c))); if (c instanceof Document) return new JQLite([c.documentElement])
        }; f.JQLite = function (c) { this.elements = c; return this }; var e = $.fn = JQLite.prototype; e.html = function (c) {
            if ("undefined" !== typeof c) {
                var a =
                    ""; c instanceof JQLite ? c.each(function () { a += this.outerHTML }) : a = c; this.elements.forEach(function (c) { c.innerHTML = a }); return this
            ...
            if ("undefined" !== typeof a) return this.elements.forEach(function (b) {
                b.setAttribute(c,
                    a)
            }), this; var b = this.elements[0]; return b ? b.getAttribute(c) : null
        }; e.removeAttr = function (c) { this.elements.forEach(function (a) { a.removeAttribute(c) }) }; e.get = function (c) { return this.elements[c] }; e.hasClass = function (c) { var a = this.get(0); return (new RegExp(" " + c + " ")).test(" " + a.className + " ") }; e.addClass = function (c) { this.elements.forEach(function (a) { a.classList ? a.classList.add(c) : $(a).hasClass(c) || (a.className += " " + c) }); return this }; e.removeClass = function (a) {
            this.elements.forEach(function (c) {
                if (c.classList) c.classList.remove(a);
                else { var b = " " + c.className.replace(/[\t\r\n]/g, " ") + " "; if ($(c).hasClass(a)) { for (; 0 <= b.indexOf(" " + a + " ");)b = b.replace(" " + a + " ", " "); c.className = b.replace(/^\s+|\s+$/g, "") } }
            }); return this
        }; e.toggleClass = function (a) { this.each(function (c, b) { b = $(b); b.hasClass(a) ? b.removeClass(a) : b.addClass(a) }) }; e.hasClass = function (a) { return (new RegExp(" " + a + " ")).test(" " + this.get(0).className + " ") }; e.children = function () {
            var a = [], b; this.elements.forEach(function (c) { b = Array.prototype.slice.call(c.children); a = a.concat(b) });
            return new JQLite(a)
        }; e.val = function (a) { if (void 0 !== a) this.each(function () { this.value = a }); else { var c = this.get(0); return c ? c.value : null } }; e.parent = function () { var a = [], b; this.elements.forEach(function (c) { (b = c.parentElement) && -1 === a.indexOf(b) && a.push(b) }); return new JQLite(a) }; e.parents = function (a) { if (!a) return this.parent(); for (var c = $(this.get(0).parentElement); c.size() && !c.is(a);)c = c.parent(); return c }; e.find = function (a) {
            var c = [], b, d; this.elements.forEach(function (f) {
                b = f.querySelectorAll(a); d = Array.prototype.slice.call(b);
                d.forEach(function (a) { -1 === c.indexOf(a) && c.push(a) })
            }); return new JQLite(c)
        }; e.is = function (a) { if (0 === this.elements.length) return !1; var c = !0; this.elements.forEach(function (b) { var f = !1; if (a.nodeType) f = b === a; else for (var e = "string" === typeof a ? d.querySelectorAll(a) : a.elements, k = e.length; k--;)e[k] === b && (f = !0); c = f && c }); return c }; e.has = function (a) { var c = []; a = a instanceof Node ? a : a.get(0); this.each(function () { this.contains(a) && c.push(this) }); return new JQLite(c) }; e.remove = function () { this.elements.forEach(function (a) { try { a.remove() } catch (k) { a.parentNode.removeChild(a) } }) };
    ...
    e.outerHeight = function () { var a = b(this.css("marginTop")) + b(this.css("marginBottom")); return Math.ceil(this.get(0).offsetHeight + a) }; e.offset = function () { var a = this.get(0); var b = a && a.ownerDocument; var d = b.documentElement; a = a.getBoundingClientRect(); b = b.defaultView; return { top: a.top + b.pageYOffset - d.clientTop, left: a.left + b.pageXOffset - d.clientLeft } }; e.position = function () {
        var a = this.get(0); var d = { top: 0, left: 0 }; if ("fixed" === this.css("position")) a = a.getBoundingClientRect(); else {
            var f = new JQLite([a.offsetParent]);
            a = this.offset(); "html" !== this.get(0).nodeName && (d = f.offset()); d.top += b(f.css("borderTopWidth")); d.left += b(f.css("borderLeftWidth"))
        } return { top: a.top - d.top - this.css("marginTop"), left: a.left - d.left - this.css("marginLeft") }
    }; e.text = function (a) { if ("undefined" !== typeof a) return this.elements.forEach(function (b) { b.innerText = a }), this; text = ""; this.elements.forEach(function (a) { text += a.innerText }); return text }; e.size = function () { return this.elements.length }; e.data = function (a, b) {
        var c = this.get(0).dataset; c = c ||
            {}; if (a && b) c[a] = b; else return a ? c[a] : c
        ...
        var b = {
            success: function () { },
            error: function () { }, url: f.location.href, method: "GET", complete: function () { }, beforeSend: function () { }, dataType: "html", data: "", contentType: "application/x-www-form-urlencoded"
        }; $.extend(b, a); q(b)
    }; e.load = function (a, b, d) { var c, f = this, e = a.indexOf(" "); if (-1 < e) { var m = a.slice(e).trim(); a = a.slice(0, e) } b && "object" === typeof b && (c = "POST"); $.ajax({ url: a, method: c || "GET", dataType: "html", data: b, success: function (a) { f.html(m ? $("<div>").append(a).find(m) : a); d && d(a) } }) }; $.get = function (a, b, d) {
        $.ajax($.extend({ url: a, success: b },
            d))
    }; $.post = function (a, b, d) { $.ajax({ url: a, data: b, method: "POST", success: d }) }
})(); var app = window.app || {}; app.event = function () { function a() { this.events = {} } var b = a.prototype; b.on = function (a, b) { var d = this.events[a] || []; d.push(b); this.events[a] = d }; b.trigger = function (a, b) { (this.events[a] || []).forEach(function (a) { a.apply(null, b) }) }; return new a }(); Number.prototype.toCommaFormat = function () { var a = this.toString().split("."); a[0] = a[0].replace(/\B(?=(\d{3})+(?!\d))/g, ","); return a.join(".") }; String.prototype.replaceAll = function (a, b) { return this.replace(new RegExp(a, "g"), b) };
...
function shareOnSocialMedea(a, b, d, f) {
    switch (a) {
        case "twitter": window.open("https://twitter.com/intent/tweet?text=" + d + " " + encodeURIComponent(b), "sharertwt", "toolbar=0,status=0,width=640,height=445"); break; case "facebook": window.open("http://www.facebook.com/sharer.php?u=" + b, "sharer", "toolbar=0,status=0,width=660,height=445"); break; case "linkedin": window.open("https://www.linkedin.com/shareArticle?mini=true&url=" + b + "title=" + d, "sharer", "toolbar=0,status=0,width=660,height=445"); break; case "google-plus": window.open("https://plus.google.com/share?url=" +
...
var cart = {
    add: function (a, b) {
        var d = this; $.ajax({
            url: "checkout/cart/add", method: "post", data: "product_id=" + a + "&quantity=" + ("undefined" != typeof b ? b : 1), dataType: "json", beforeSend: function () { $("#cart > button").button("loading") }, complete: function () { $("#cart > button").button("reset") }, success: function (b) {
                b.redirect && (location = b.redirect); b.success && (showMsg(b.success), setTimeout(function () {
                    $(".mc-toggler .text").html(b.total); $(".mc-toggler .counter").text(b.count).attr("data-count", b.count); $(".mc-toggler").attr("title",
                        b.total)
                }, 100), d.reload(), app.event.trigger("addToCart", b), fbq("track", "AddToCart", { content_ids: [a], content_type: "product", value: b.item_total, currency: "BDT" }), window.gtag && gtag("event", "add_to_cart", { items: [{ item_name: b.name, item_id: b.product_id, price: b.price, currency: "BDT" }] }))
            }
        })
    }, update: function (a, b) {
        var d = this; $.ajax({
            url: "checkout/cart/edit", method: "post", data: "ajax=true&quantity[" + encodeURIComponent(a) + "]=" + ("undefined" != typeof b ? b : 1), dataType: "json", beforeSend: function () { $("#cart > button").button("loading") },
            complete: function () { $("#cart > button").button("reset") }, success: function (a) { "checkout/cart" == getURLVar("route") || "checkout/checkout" == getURLVar("route") ? location = "checkout/cart" : ($(".mc-toggler .text").html(a.total), $(".mc-toggler .counter").text(a.count).attr("data-count", a.count), $(".mc-toggler").attr("title", a.total), d.reload()) }
        })
    }, remove: function (a, b) {
        var d = this; $.ajax({
            url: "checkout/cart/remove", method: "post", data: "key=" + a, dataType: "json", beforeSend: function () { $("#cart > button").button("loading") },
            complete: function () { $("#cart > button").button("reset") }, success: function (a) { setTimeout(function () { $(".mc-toggler .text").html(a.total); $(".mc-toggler .counter").text(a.count).attr("data-count", a.count); $(".mc-toggler").attr("title", a.total) }, 100); b ? location = "checkout/cart" : d.reload() }
        })
    }, _bulk_add: function (a) {
        var b = this; $.ajax({
            url: "checkout/cart/bulk_add", method: "post", data: a, dataType: "json", contentType: a instanceof FormData ? !1 : "application/x-www-form-urlencoded", beforeSend: function () { $("#cart > button").button("loading") },
            complete: function () { $("#cart > button").button("reset") }, success: function (a) { var d = new Popup(a.message, "mgs-popup "); d.render(); d.el.find("#form-cart").on("submit", function (a) { a.preventDefault(); d.close(); b._bulk_add(new FormData(this)) }); a.success && (setTimeout(function () { $(".mc-toggler .text").html(a.total); $(".mc-toggler .counter").text(a.count).attr("data-count", a.count); $(".mc-toggler").attr("title", a.total) }, 100), b.reload(), app.event.trigger("addToCart", a)) }
        })
    }, bulk_add: function (a) {
        var b = {};
        a.forEach(function (a, f) { b["product_id[" + f + "]"] = a }); this._bulk_add(b)
    }, reload: function () { var a = $("#m-cart"); a.append($("<div class='mask'></div>")); $.get("common/cart/info", function (b) { b = $(b); a.find(".content").size() ? (a.find(".content").html(b.find(".content").html()), a.find(".footer").html(b.find(".footer").html())) : a.html(b); a.find(".mask").remove(); app.event.trigger("cart_load", [a]) }) }
}, voucher = {
    add: function () { }, remove: function (a) {
        $.ajax({
            url: "checkout/cart/remove", method: "post", data: "key=" + a,
            dataType: "json", beforeSend: function () { $("#cart > button").button("loading") }, complete: function () { $("#cart > button").button("reset") }, success: function (a) { setTimeout(function () { $("#cart > button").html('<span id="cart-total"><i class="fa fa-shopping-cart"></i> ' + a.total + "</span>") }, 100); "checkout/cart" == getURLVar("route") || "checkout/checkout" == getURLVar("route") ? location = "checkout/cart" : cart.reload() }
        })
    }
}, wishlist = {
    add: function (a) {
        $.ajax({
            url: "account/wishlist/add", method: "post", data: "product_id=" +
    ...
    add: function (a) {
        $.ajax({
            url: "common/compare/add", method: "post", data: "ajax=true&product_id=" + a, dataType: "json",
            success: function (a) { $(".alert").remove(); a.success && (showMsg(a.success), $(".cmpr-toggler .text").html(a.total), $(".cmpr-toggler .counter").text(a.count), compare.reload()) }
        })
    }, remove: function (a) { var b = this; $.ajax({ url: "common/compare/remove", method: "post", data: "product_id=" + a, dataType: "json", success: function (a) { setTimeout(function () { $(".cmpr-toggler .text").html(a.total); $(".cmpr-toggler .counter").text(a.count) }, 100); b.reload() } }) }, clear: function () {
        $.ajax({
            url: "common/compare/clear", method: "post",
            dataType: "json", success: function (a) { setTimeout(function () { $(".cmpr-toggler .text").html(a.total); $(".cmpr-toggler .counter").text(a.count) }, 100); compare.reload() }
        })
    }, reload: function () { var a = $("#cmpr-panel"); a.append($("<div class='mask'></div>")); $.get("common/compare/index", function (b) { b = $(b); a.find(".content").html(b.find(".content").html()); a.find(".footer").html(b.find(".footer").html()); a.find(".mask").remove() }) }
}, stock_request = {
    add: function (a) {
        $.ajax({
            url: "checkout/stock_request/add", method: "post",
            data: { product_id: a, referrer: location.href }, dataType: "json", beforeSend: function () { }, complete: function () { }, success: function (a) { a.redirect && (location = a.redirect); a.success && showMsg(a.success, "success"); a.error && showMsg(a.error, "error") }
        })
    }
}, quote_request = {
    add: function (a, b) {
        $.ajax({
            url: "product/product/quote", method: "post", data: "product_id=" + a + "&quantity=" + ("undefined" != typeof b ? b : 1), dataType: "json", beforeSend: function () { $("#button-cart").button("loading") }, complete: function () { $("#button-cart").button("reset") },
...
$.fn.autocomplete = function (a) {
    return this.each(function () {
        var b = $(this), d = this; this.timer = null; this.items = []; this.wrap = '<ul class="dropdown-menu"></ul>'; $.extend(this, a); b.attr("autocomplete", "off"); b.on("focus", function () { d.request() }); b.on("blur", function () { setTimeout(function (a) { a.hide() }, 200, this) }); b.on("keydown", function (a) { switch (a.keyCode) { case 27: d.hide(); break; default: d.request() } }); d.click = function (a) {
            (value = $(a.target).parent().attr("data-value")) && this.items[value] && (a.preventDefault(),
                this.select(this.items[value]))
        }; d.show = function () { var a = b.position(), d = b.siblings(".dropdown-menu"); d.css({ top: a.top + b.outerHeight() + "px", left: a.left + "px" }); d.show() }; d.hide = function () { b.siblings(".dropdown-menu").hide() }; this.request = function () { clearTimeout(d.timer); d.timer = setTimeout(function (a) { a.source($(a).val(), a.response.bind(a)) }, 500, this) }; d.response = function (a) {
            var f = ""; if (a.length) {
                for (i = 0; i < a.length; i++)d.items[a[i].value] = a[i]; for (i = 0; i < a.length; i++)a[i].category || (f += '<li data-value="' +
                    a[i].value + '"><a href="#">' + a[i].label + "</a></li>")
            } f ? d.show() : d.hide(); b.siblings(".dropdown-menu").html(f)
        }; b.after(this.wrap); b.siblings(".dropdown-menu").delegate("a", "click", this.click.bind(this))
    })
}; $.fn.button = function (a) { 0 !== this.size() && ("loading" === a ? (this.addClass("disabled"), this.attr("disabled", ""), this.data("old_text", this.text()), this.text("Loading...")) : (this.removeClass("disabled"), this.removeAttr("disabled"), this.text(this.data("old_text")))) };
...
_t.activate = function (a) { this.active && (this.active.removeClass("active"), this.elm.find("#" + this.active.attr("data-tab")).hide()); this.elm.find("#" + a.attr("data-tab")).show(); a.addClass("active"); this.active = a }; (function () {
    function a(a, b) { var d = new Image, e = a.getAttribute("data-src"); d.onload = function () { a.parent ? a.parent.replaceChild(d, a) : a.src = e; b ? b() : null }; d.src = e } function b(a) { a = a.getBoundingClientRect(); return 0 <= a.top && 0 <= a.left && a.top <= (window.innerHeight || document.documentElement.clientHeight) } var d = [], f = function () { for (var f = [], h = 0; h < d.length; h++)b(d[h]) && (a(d[h]), f.push(d[h])); f.forEach(function (a) { a = d.indexOf(a); -1 < a && d.splice(a, 1) }) }; $(function () {
        $("img.lazy").each(function () { d.push(this) }); f();
        window.addEventListener("scroll", f)
    })
})(); $(function () {
    function a(a) { var b = ""; a.forEach(function (a) { b = "remainder_cnt" === a.type ? b + ('<div class="search-item remainder-count"><a href="' + a.href + '">' + a.label + "</a></div>") : b + ('<div class="search-item"><a href="' + a.href + '"><div class="image"><img src="' + a.thumb + '"></div><div class="name">' + a.label + "</div>" + (a.price ? a.special ? '<div class="price"><ins>' + a.special + "</ins> &nbsp; <del>" + a.price + "</del></div>" : '<div class="price">' + a.price + "</div>" : "") + "</a></div>") }); return b } var b = $('#search input[name="search"]'),
        ...
        }); var f = new Tab(d), g = !1, h = b.get(0), l = h.hide; h.hide = function () { g || l() }; $(document).on("click",
            function (a) { a = $(a.target); d.has(a).size() || b.is(a) ? g = !0 : (g = !1, h.hide()) })
}); $(function () {
    ...
    var g = $("html"), h = $("body"), l = $(".overlay"), e = a.prototype; e.show = function (a) { this.active && this.hide(this.active); var b = $(a.target), c = $(a.toggle); a.overlay && l.addClass("open"); a.no_scroll && $("body").addClass("no-scroll"); c.addClass("close"); b.addClass("open"); this.active = a }; e.hide = function (a) { if (a = a || this.active) { var b = $(a.target); $(a.toggle).removeClass("close"); b.removeClass("open"); l.removeClass("open"); $("body").removeClass("no-scroll"); this.active = a } }; var q = new a([{
        ...
        overlay: !0, no_scroll: !0
    }, { toggle: ".mc-toggler", target: "#m-cart" }, { toggle: ".cmpr-toggler", target: "#cmpr-panel" }, { toggle: ".search-toggler", target: "#search" }, { toggle: "#lc-toggle, .lc-close", target: "#column-left", overlay: !0, no_scroll: !0 }]); l.on("click", function () { q.hide() }); app.event.on("cart_load", function (a) {
        a.find(".button-coupon").on("click", function () {
            var a = $(this), b = $(a.data("target")); $.ajax({
                url: "checkout/coupon/coupon", method: "post", data: "coupon=" + encodeURIComponent(b.val()), dataType: "json",
                beforeSend: function () { a.button("loading", "Applying ...") }, complete: function () { a.button("reset") }, success: function (a) { a.error && showMsg('<div class="msg-wrap">' + a.error + "</div>", "error"); a.success && cart.reload() }
            })
        })
    }); app.event.on("addToCart", function () { $(".mc-toggler").addClass("bounce"); setTimeout(function () { $(".mc-toggler").removeClass("bounce") }, 900) }); $(".has-child a").on("click", function (a) {
        $(this).parent().toggleClass("open"); var b = $(a.target); (isMobile() && b.is(".has-child > a") || b.is(".responsive-menu > .has-child > a")) &&
            a.preventDefault()
        ...
        function () { 132 < g.scrollTop() ? h.addClass("on-scroll") : h.removeClass("on-scroll") }); $(".alert .close").on("click", function () { $(this).parent(".alert").remove() }); $("[data-area]").on("click", function (a) { a.preventDefault(); a = $(this); $("html,body").scrollTo($("#" + a.data("area")).offset().top - 140, 600) }); var c = (new Date).getTime(); e = localStorage.showed; var k = localStorage.lastVisited, p = []; e = parseInt(e); e = isNaN(e) ? null : e; k = parseInt(k); k = isNaN(k) ? null : k; var r = app.popupDuration ? app.popupDuration : 12; app.enablePopup &&
            k && 36E4 > c - k && (!e || c - e > 36E5 * r) && p.push(1); $(".ads:not(.loaded)").each(function () { p.push($(this).attr("data-position")) }); if (false && p.length) { var t = "device_type=" + (isMobile() ? 1 : 3); p.forEach(function (a) { t += "&ads_position[]=" + a }); $.ajax({ url: "/api/ads", data: t, method: "post", dataType: "json", success: function (a) { a.forEach(function (a) { d(a) }); (a = a[1]) && a.image && d(a.image, a.title, a.url) } }) } e = (new Date).getHours(); 9 <= e && 21 >= e && $(".svg-icon svg").show(); localStorage.lastVisited = c; $(".cmpr-product").autocomplete({
                source: function (a,
